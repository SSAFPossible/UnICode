<template>
  <div>
    ...
    <div class="container">
      <div class="title">프로젝트 상세 정보</div>
      <div class="content">
        ...
        <b-form-input
          type="text"
          id="url"
          v-model="exhibitBoard.project.url"
          trim
        />
        <div class="subtitle">태그</div>
        <b-form-input
          type="text"
          v-model="exhibitBoard.project.language.name"
          id="name"
          trim
        />
        ...
        <b-form-input
          type="text"
          v-model="exhibitBoard.project.member.uid"
          id="member"
          trim
        />
        ...
        <b-form-file
          ...
          v-model="fileList"
        ></b-form-file>
        <b-button class="submit" @click="update">작성</b-button>
        <b-button class="delete" @click="del">삭제</b-button>
        <b-button class="like" @click="like(exhibitBoard.bid)">좋아요</b-button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async update() {
      ...
      await this.modExhibitBoard(params);
      this.moveTo();
    },
    moveTo() {
    ...
    del() {
      const data = {
        bid: this.exhibitBoard.bid,
        uid: sessionStorage.getItem("user"),
      };
      this.delExhibitBoard(data);
      this.$router.push({ name: "project" });
    },
    like(bid) {
      ...
      console.log("like");
      this.setExhibitLike(params);
    },
  },
  computed: {
    ...mapState(projectStore, ["projectList", "exhibitBoard"]),
  },
  created() {
    this.inputs = this.exhibitBoard;
  },
};
</script>
